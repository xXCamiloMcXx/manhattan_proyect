...
import com.ingeniawork.ticketflow.domain.boleta.DatoObtenerBoleta;
...
public class BoletaController {
    ...
    @Transactional
    @PostMapping
    public ResponseEntity registrar(@RequestBody @Valid DatosCreacionBoleta datosCreacionBoleta, HttpServletRequest request){
        ...
        var boleta = new Boleta(token,usuario,tipoDeBoleta,evento,datosCreacionBoleta);
    ...
    @GetMapping()
    public ResponseEntity listar (@PageableDefault(size = 5, sort = {"fecha_compra"}) Pageable paginacion, HttpServletRequest request){
        var usuarioId = recuperarIdService.obtenerId(request);
        var boletas = boletaRepository.findByUsuarioId(usuarioId).stream()
                .map(DatosDetalleBoleta::new).toList();
        return ResponseEntity.ok(boletas);
    }

    @Transactional
    @PutMapping("/{token}")
    public ResponseEntity VerificarBoleta (@PathVariable String token){
        var boletaBefore = boletaRepository.findByToken(token);
        var boletaAfter = boletaBefore;
        boletaAfter.ActualizarEstado(boletaBefore);
        return ResponseEntity.ok(new DatosDetalleBoleta(boletaBefore));
    }
